...
class Node(object):
    ...
    def __mul__(self, other):
        """TODO: Your code here"""
        """Multiple two nodes return a new node."""
        if isinstance(other, Node):
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if len(node.inputs) > 0:
                input_val = [node_to_val_map[input] for input in node.inputs]
                result = node.op.compute(node, input_val)
...
def gradients(output_node, node_list):
    ...
    """TODO: Your code here"""
    # node_to_output_grad[output_node] =
